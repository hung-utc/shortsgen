      ...
      async fetch(req) {
        ...
        if (pathname !== "/") {
          ...
          if (await file.exists()) {
            ...
            if (pathname === "/sw.js") {
              // Never let browsers/proxies cache the service worker script:
              // a stale sw.js pins users to old bundles indefinitely (hard
              // refresh does not clear Service-Worker Cache Storage).
              // Hashed /assets/* stay long-cacheable — untouched by this.
              return new Response(file, {
